...
const puntEstacio=[42.39758, 2.15303];
...
var map = L.map('map').setView(puntPuigmal,12);
...
/*
  Exmpemple de punt amb propietats

    var track =  {
        "type":"Feature",
        "geometry": {
          "type":"Point","coordinates":[2.1222648,42.3862]},
        "properties":{
          "id":9,
          "name": "nom del track",
          "amenity": "????",
          "time":"2021-11-17T22:34:16.000+00:00",
          "popupContent": "punt del track"        
        }
          }
  

    L.geoJSON(track, { onEachFeature: onEachFeature  }).addTo(map);
    L.geoJSON(track, { onEachFeature: onEachFeature  }).addTo(map)
      .bindPopup(track.properties.time);    // Sobreescriu el popupConent indicat al geoJSON
      //.bindPopup(usuarisList[0].nom + " " + usuarisList[0].cognoms)

//    L.geoJSON(geojsonFeature).addTo(map);
//    L.geoJSON(track).addTo(map).bindPopup(track.properties.time);
*/

// Definició de waypoints de referència
// Estació: [42.39758, 2.15303]
// Puigmal: [42.3832679, 2.1167833]
// Pluviòmetre: [42.3906, 2.1242]
function dibuixaPuntsClau() {
  // Afegeix punt addicionals al track de la crusa, sobre el mapa     
      paintPosition('1','42.3975800','2.1530300','<h3>Punt de sortida</h3>Vall de Núria' );    
      paintPosition('2','42.3832679','2.1167833','<h3>Puigmal</h3>Control de cim' );    
      paintPosition('3','42.4002800','2.1476400','<h3>Pont de ciment. Torrent de finestrelles</h3>No hi ha neu.' );    
}      
      
// Bloc de funcions utilitzades
function paintPosition(n,lat, lng, desc)    
{
